...
export default function ClientWallets() {
  ...
  const handleLink = async (e) => {
    ...
    if (!address.startsWith("0x") && chain !== "solana") {
      showToast("Invalid EVM address", "error");
      return;
    }
  ...
  const handleUnlink = async (id) => {
    if (
      !window.confirm(
        "Are you sure you want to unlink this wallet? Chain Forge will stop tracking its history.",
      )
    )
      return;
    try {
    ...
    <main className="relative mx-auto max-w-[1200px] px-6 py-10 min-h-screen">
      ...
      <header className="mb-10">
        ...
        <p className="text-slate-400 mt-2 max-w-2xl text-sm leading-relaxed">
          Link and configure your blockchain wallets here. Once integrated,
          Chain Forge will automatically track transactions and trigger webhooks
          for these addresses across multiple chains.
        </p>
      </header>
      ...
      >
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          >
            <div className="bg-[#0f131b] rounded-[15px] py-4 px-6 flex items-center justify-between">
              <span>
                {showAddForm ? "Cancel Integration" : "+ Add New Wallet"}
              </span>
              <span className="material-symbols-outlined">
                {showAddForm ? "close" : "account_balance_wallet"}
              </span>
            </div>
          </motion.button>
          ...
          >
            ...
            <div className="space-y-4">
              <div className="flex gap-3">
                <span className="flex-shrink-0 w-6 h-6 rounded-full bg-violet-500/20 text-violet-400 flex items-center justify-center text-[10px] font-bold">
                  1
                </span>
                <p className="text-xs text-slate-400 leading-tight">
                  Link your primary business wallet to start tracking incoming
                  payments.
                </p>
              </div>
              <div className="flex gap-3">
                ...
                <p className="text-xs text-slate-400 leading-tight">
                  Each wallet can be configured for a specific chain (Ethereum,
                  Polygon, etc.).
                </p>
              </div>
            </div>
        ...
        <div className="lg:col-span-2">
          <motion.div
            variants={item}
            className="rounded-2xl bg-white/5 border border-white/10 backdrop-blur-xl overflow-hidden min-h-[400px]"
          >
              ...
              {wallets.length === 0 ? (
                <div className="py-20 text-center">
                  ...
                  <p className="text-slate-500 text-sm">
                    No wallets integrated yet.
                  </p>
                  <button
                    onClick={() => setShowAddForm(true)}
                    className="mt-4 text-violet-400 text-xs font-bold hover:underline uppercase"
                  >
                    Integrate your first wallet now
                  </button>
                </div>
              ) : (
                wallets.map((w) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="rounded bg-slate-800 px-1.5 py-0.5 text-[10px] font-bold text-slate-400 uppercase tracking-tighter">
                            {w.chain}
                          </span>
                        </div>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="text-right mr-4 hidden sm:block">
                        <p className="text-[10px] uppercase font-bold text-slate-600 mb-0.5">
                          Integration Status
                        </p>
